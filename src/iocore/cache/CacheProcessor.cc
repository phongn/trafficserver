...
CacheProcessor::cacheInitialized()
{
  ...
  if (caches_ready) {
    ...
    if (gnstripes) {
      ...
      static const char *const ram_cache_algorithm_name[] = {"CLFUS", "LRU", "W-TinyLFU"};
      int const                ram_alg                    = cache_config_ram_cache_algorithm;
      Dbg(dbg_ctl_cache_init, "ram_cache algorithm = %d (%s)", ram_alg,
          (ram_alg >= 0 && ram_alg <= RAM_CACHE_ALGORITHM_WTINYLFU) ? ram_cache_algorithm_name[ram_alg] : "unknown");
      for (int i = 0; i < gnstripes; i++) {
        ...
        case RAM_CACHE_ALGORITHM_LRU:
          ...
          break;
        case RAM_CACHE_ALGORITHM_WTINYLFU:
          gstripes[i]->ram_cache = new_RamCacheWTinyLFU();
          break;
        }
      }
