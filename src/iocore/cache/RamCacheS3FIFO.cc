...
// holds keys, so it costs per-entry metadata at high object cardinality. Sized by bytes to fit the
...
RamCacheS3FIFO::size() const
{
  ...
  for (int seg = 0; seg < 3; seg++) {
    forl_LL(RamCacheS3FIFOEntry, e, _seg[seg])
    {
      s += sizeof(*e);
      if (e->data) {
        s += sizeof(*e->data);
...
RamCacheS3FIFO::_evict_main()
{
  while (_seg[SEG_MAIN].head) {
    ...
    if (c->freq >= 1) {
      ...
      _seg[SEG_MAIN].remove(c);
      _m_bytes -= ENTRY_OVERHEAD + c->size;
      ts::Metrics::Gauge::decrement(cache_rsb.ram_cache_bytes, ENTRY_OVERHEAD + c->size);
      ts::Metrics::Gauge::decrement(_stripe->cache_vol->vol_rsb.ram_cache_bytes, ENTRY_OVERHEAD + c->size);
      _unlink_hash(c);
      c->data = nullptr;
      ramCacheS3FIFOEntryAllocator.free(c);
      _objects--;
      _nentries--;
      return;
    }
...
RamCacheS3FIFO::put(CryptoHash *key, IOBufferData *data, [[maybe_unused]] uint32_t len, bool, uint64_t auxkey)
{
  ...
  uint32_t             i  = key->slice32(3) % _nbuckets;
  RamCacheS3FIFOEntry *ne = ramCacheS3FIFOEntryAllocator.alloc();
  ne->key                 = *key;
  ne->auxkey              = auxkey;
  ne->size                = size;
  ne->freq                = 0;
